-- Script para corregir error de array literal malformado
-- Este script soluciona el error: "malformed array literal: "[]""

-- Problema identificado:
-- Los arrays se enviaban como strings JSON ("[]") en lugar de arrays reales
-- PostgreSQL no puede convertir directamente strings como "[]" a TEXT[]

-- Solución: Función helper para convertir JSONB a TEXT[]
CREATE OR REPLACE FUNCTION public.jsonb_to_text_array(jsonb_data JSONB) 
RETURNS TEXT[] 
LANGUAGE plpgsql
AS $$
BEGIN
    IF jsonb_data IS NULL OR jsonb_data = 'null'::jsonb THEN
        RETURN '{}'::TEXT[];
    ELSIF jsonb_typeof(jsonb_data) = 'array' THEN
        RETURN ARRAY(SELECT jsonb_array_elements_text(jsonb_data));
    ELSE
        RETURN '{}'::TEXT[];
    END IF;
END;
...
-- Función create_service_simple actualizada con manejo correcto de arrays
...
-- Función update_service_simple actualizada con manejo correcto de arrays
...
-- Prueba de la función con arrays
SELECT 'Prueba con arrays válidos:' as test_name;
SELECT public.create_service_simple('{
  "title": "Servicio de Prueba con Arrays",
  "description": "Servicio de prueba con arrays",
  "price": 50.00,
  "difficulty_level": "facil",
  "category_id": "actividades",
  "subcategory_id": "aventura",
  "what_to_bring": ["Cámara", "Ropa cómoda"],
  "included_services": ["Guía", "Transporte"],
  "not_included_services": ["Comida", "Bebidas"],
  "dietary_options": ["Vegetariano", "Vegano"]
}'::jsonb) as result;

-- Prueba de la función con arrays vacíos
SELECT 'Prueba con arrays vacíos:' as test_name;
SELECT public.create_service_simple('{
  "title": "Servicio de Prueba con Arrays Vacíos",
  "description": "Servicio de prueba con arrays vacíos",
  "price": 75.00,
