...
-- SOLUCIONA el error "CASE types text[] and jsonb cannot be matched"
...
-- =====================================================
-- 2. CREAR FUNCIÓN CON TIPOS CORRECTOS (INCLUYENDO text[])
-- =====================================================
...
BEGIN
    ...
    UPDATE services SET
        ...
        characteristics = CASE
            WHEN service_data->>'characteristics' IS NOT NULL THEN 
                CASE 
                    WHEN jsonb_typeof(service_data->'characteristics') = 'array' 
                    THEN (service_data->'characteristics')::TEXT[]
                    ELSE ARRAY[service_data->>'characteristics']
                END
        ...
        dietary_options = CASE
            WHEN service_data->>'dietary_options' IS NOT NULL THEN 
                CASE 
                    WHEN jsonb_typeof(service_data->'dietary_options') = 'array' 
                    THEN (service_data->'dietary_options')::TEXT[]
                    ELSE ARRAY[service_data->>'dietary_options']
                END
        ...
        END,
        what_to_bring = CASE
            WHEN service_data->>'what_to_bring' IS NOT NULL THEN 
                CASE 
                    WHEN jsonb_typeof(service_data->'what_to_bring') = 'array' 
                    THEN (service_data->'what_to_bring')::TEXT[]
                    ELSE ARRAY[service_data->>'what_to_bring']
                END
            ELSE what_to_bring
        END,
        included_services = CASE
            WHEN service_data->>'included_services' IS NOT NULL THEN 
                CASE 
                    WHEN jsonb_typeof(service_data->'included_services') = 'array' 
                    THEN (service_data->'included_services')::TEXT[]
                    ELSE ARRAY[service_data->>'included_services']
                END
            ELSE included_services
        END,
        not_included_services = CASE
            WHEN service_data->>'not_included_services' IS NOT NULL THEN 
                CASE 
                    WHEN jsonb_typeof(service_data->'not_included_services') = 'array' 
                    THEN (service_data->'not_included_services')::TEXT[]
                    ELSE ARRAY[service_data->>'not_included_services']
                END
        ...
        pickup_locations = CASE
            WHEN service_data->>'pickup_locations' IS NOT NULL THEN 
                CASE 
                    WHEN jsonb_typeof(service_data->'pickup_locations') = 'array' 
                    THEN (service_data->'pickup_locations')::TEXT[]
                    ELSE ARRAY[service_data->>'pickup_locations']
                END
        ...
        equipment_provided = CASE
            WHEN service_data->>'equipment_provided' IS NOT NULL THEN 
                CASE 
                    WHEN jsonb_typeof(service_data->'equipment_provided') = 'array' 
                    THEN (service_data->'equipment_provided')::TEXT[]
                    ELSE ARRAY[service_data->>'equipment_provided']
                END
        ...
        guide_languages = CASE
            WHEN service_data->>'guide_languages' IS NOT NULL THEN 
                CASE 
                    WHEN jsonb_typeof(service_data->'guide_languages') = 'array' 
                    THEN (service_data->'guide_languages')::TEXT[]
                    ELSE ARRAY[service_data->>'guide_languages']
                END
        ...
        END,
        images = CASE
            WHEN service_data->>'images' IS NOT NULL THEN 
                CASE 
                    WHEN jsonb_typeof(service_data->'images') = 'array' 
                    THEN (service_data->'images')::TEXT[]
                    ELSE ARRAY[service_data->>'images']
                END
            ELSE images
        END,
        age_ranges = CASE
...
BEGIN
    SELECT update_service_with_age_ranges(
        ...
        '{"title": "Test Service", "price": 100, "duration": 120}'::JSONB
    ) INTO test_result;
...
BEGIN
    ...
    RAISE NOTICE '✅ Tipos de datos corregidos (text[] vs JSONB)';
