-- Script SIMPLE para corregir el error de creación de conversaciones (V2)
...
-- =====================================================
-- PASO 8: INSERTAR CATEGORÍAS POR DEFECTO (MEJORADO)
-- =====================================================
...
BEGIN
  -- Insertar General
  IF NOT EXISTS (SELECT 1 FROM chat_categories WHERE name = 'General') THEN
    INSERT INTO chat_categories (name, description, color, icon, sort_order) 
    VALUES ('General', 'Consultas generales y soporte básico', '#0061A8', 'message-circle', 1);
  END IF;

  -- Insertar Técnico
  IF NOT EXISTS (SELECT 1 FROM chat_categories WHERE name = 'Técnico') THEN
    INSERT INTO chat_categories (name, description, color, icon, sort_order) 
    VALUES ('Técnico', 'Problemas técnicos y bugs', '#DC2626', 'wrench', 2);
  END IF;

  -- Insertar Facturación
  IF NOT EXISTS (SELECT 1 FROM chat_categories WHERE name = 'Facturación') THEN
    INSERT INTO chat_categories (name, description, color, icon, sort_order) 
    VALUES ('Facturación', 'Consultas sobre facturación y pagos', '#059669', 'credit-card', 3);
  END IF;

  -- Insertar Reservas
  IF NOT EXISTS (SELECT 1 FROM chat_categories WHERE name = 'Reservas') THEN
    INSERT INTO chat_categories (name, description, color, icon, sort_order) 
    VALUES ('Reservas', 'Consultas sobre reservas y tours', '#7C3AED', 'calendar', 4);
  END IF;

  -- Insertar Sugerencias
  IF NOT EXISTS (SELECT 1 FROM chat_categories WHERE name = 'Sugerencias') THEN
    INSERT INTO chat_categories (name, description, color, icon, sort_order) 
    VALUES ('Sugerencias', 'Sugerencias y mejoras', '#EA580C', 'lightbulb', 5);
  END IF;
END $$;
