...
export default function ServicesManagementFixed() {
  ...
  const getSupabaseClient = useCallback(async () => {
    ...
    try {
      const { getSupabaseClient: getClient } = await import("@/lib/supabase-unified")
  ...
  }, [isClient, getSupabaseClient])

  useEffect(() => {
    if (isClient) {
      loadServices()
    }
  }, [isClient, loadServices])

  const handleToggleAvailability = useCallback(async (serviceId: string, currentStatus: boolean) => {
    if (!isClient) return
